...
while True:
    try:
        preco_atual_str = input("Informe o preço atual do produto (ou 'S' para sair): ").strip()
        if preco_atual_str.upper() == 'S':
            break
    ...
    # Calcular novo preço baseado nas condições
    if venda_mensal < 500 and preco_atual < 30:
        ...
        preco_novo = preco_atual  # Sem alteração se condições não atendidas

    print(f"Novo preço do produto: {preco_novo:.2f}")
    
    continue_input = input("Deseja calcular outro preço? (S/N): ")
